...
if [ "$#" -ne 4 ]; then
    echo "Usage: $0 <GRAFANA_INSTANCE> <API_KEY> <TARGET_ORG_ID> <INPUT_JSON>"
    exit 1
fi
...
for datasource in $(echo "$DATA" | jq -c '.[]'); do
    ...
    response=$(curl -s -o /dev/null -w "%{http_code}" -X GET "$TARGET_DATASOURCES_ENDPOINT/name/$datasource_name" -H "Authorization: Bearer $API_KEY" -d "orgId=$TARGET_ORG_ID")
    
    if [ "$response" == "200" ]; then
        echo "Datasource '$datasource_name' already exists in organization '$TARGET_ORG_ID'. Skipping."
    else
        ...
        if [[ $curl_response == *"Datasource added"* ]]; then
            echo "Datasource '$datasource_name' added to organization '$TARGET_ORG_ID'."
        else
            echo "Failed to add datasource '$datasource_name' to organization '$TARGET_ORG_ID'."
            echo "$curl_response"
        fi
    fi
done